...
import { MpesaFlowDefaultError } from "../models/errors/mpesaflowdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | MpesaFlowDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | MpesaFlowDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new MpesaFlowDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new MpesaFlowDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
